...
class FileService:
    def save_file(
        self, db: Session, file: UploadFile, owner_id: int, content_type: str
    ) -> FileModel:
    ...
    def get_file_by_id(
        ...
        try:
            ...
            if not db_file:
                logger.warning(
                    "File with id %s not found in database", file_id
                )
                raise HTTPException(
    ...
    def get_task_status(
        self, task_id: str, db: Session, current_user: User
    ) -> dict:
    ...
    def start_image_conversion(
        ...
        try:
            ...
            logger.error(
                "Error processing file %s: %s", file.filename, e, exc_info=True
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
