...
class TestFile(BaseModel):
    ...
    @classmethod
    def create_test_image(cls, width: int = 100, height: int = 100, color: str = "black") -> 'TestFile':
    ...
    def create_test_pdf(cls, page_count: int = 1) -> 'TestFile':
        ...
        offset = 0
        for i in range(1, 3 + page_count):  # +3 for catalog, pages, and each page
...
class TestFileOperations:
    ...
    def test_upload_file_parameterized(
        ...
        test_file = TEST_FILES.get(file_type, {"filename": "nonexistent.txt", "content": b"", "content_type": "text/plain", "size": 0})
    ...
    def test_download_file_parameterized(
        ...
        response = authorized_client.get(f"/api/v1/files/{file_id}/download")
        ...
        if expected_status == 200:
            ...
            assert f'filename="{test_file.filename}"' in response.headers["content-disposition"]
        elif expected_detail:
            assert expected_detail in response.json().get("detail", "")
    ...
    def test_delete_file_parameterized(
        ...
        if file_type == "nonexistent":
            ...
            if test_name == "already_deleted":
                del_response = authorized_client.delete(f"/api/v1/files/{file_id}")
                assert del_response.status_code == 200
        
        # Attempt to delete the file
        response = authorized_client.delete(f"/api/v1/files/{file_id}")
        ...
        if expected_status == 200:
            ...
            response = authorized_client.get(f"/api/v1/files/{file_id}")
    ...
    def test_upload_image(self, authorized_client: TestClient, test_image: bytes, db_session: Session) -> None:
        """Test uploading an image file."""
        # Prepare file for upload
        files = {"file": ("test.png", test_image, "image/png")}
        
        # Make request
        response = authorized_client.post("/api/v1/files/upload/", files=files)
    ...
    def test_upload_pdf(self, authorized_client: TestClient, test_pdf: bytes) -> None:
    ...
    def test_upload_unsupported_file_type(self, authorized_client: TestClient) -> None:
    ...
    def test_download_nonexistent_file(self, authorized_client: TestClient) -> None:
        """Test downloading a file that doesn't exist."""
        response = authorized_client.get("/api/v1/files/999999/download")
        assert response.status_code == 404
    
    def test_list_files(self, authorized_client: TestClient, test_file: FileModel) -> None:
        """Test listing user's files."""
        response = authorized_client.get("/api/v1/files/")
    ...
    def test_get_file_metadata(self, authorized_client: TestClient, test_file: FileModel) -> None:
        """Test getting file metadata."""
        response = authorized_client.get(f"/api/v1/files/{test_file.id}")
    ...
    def test_convert_image_to_pdf(
        self, authorized_client: TestClient, test_image: bytes, db_session: Session
    ) -> None:
        """Test converting an image to PDF."""
        # Upload image
        files = {"file": ("test.png", test_image, "image/png")}
        response = authorized_client.post("/api/v1/files/convert-to-pdf/", files=files)
        ...
        pdf_file = db_session.query(FileModel).filter(FileModel.id == file_id).first()
    ...
    def test_merge_pdfs(
        self, authorized_client: TestClient, test_pdf: bytes, db_session: Session
    ) -> None:
        """Test merging multiple PDFs."""
        # Upload first PDF
        files1 = {"files": ("test1.pdf", test_pdf, "application/pdf")}
        response1 = authorized_client.post("/api/v1/files/upload/", files=files1)
        file1_id = response1.json()["id"]
        
        # Upload second PDF
        files2 = {"files": ("test2.pdf", test_pdf, "application/pdf")}
        response2 = authorized_client.post("/api/v1/files/upload/", files=files2)
        ...
        merged_file = db_session.query(FileModel).filter(FileModel.id == merged_file_id).first()
