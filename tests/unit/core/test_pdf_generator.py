...
class TestPDFGenerator:
    ...
    def test_image_to_pdf_conversion_error(
        self, mock_convert: MagicMock, mock_file: MagicMock, mock_mkdir: MagicMock
    ) -> None:
        ...
        # Test & Verify
        with pytest.raises(
            ValueError, match="Failed to convert image to PDF: Invalid image data"
        ):
    ...
    def test_image_to_pdf_write_error(
        ...
        mock_file = MagicMock()
        mock_file.__enter__.return_value.write.side_effect = IOError("Disk full")
        mock_open_func.return_value = mock_file

        # Test & Verify
        with pytest.raises(ValueError, match="Failed to convert image to PDF: Disk full"):
    ...
    def test_merge_pdfs_empty_list(
        self, mock_merger_class: MagicMock, tmp_path: Path
    ) -> None:
        ...
        # Verify no merger was created or used
        mock_merger_class.assert_not_called()
        mock_merger_class.return_value.append.assert_not_called()
        mock_merger_class.return_value.write.assert_not_called()
        # close() should not be called since merger is never created
        mock_merger_class.return_value.close.assert_not_called()
    ...
    def test_image_to_pdf_with_file_object_not_at_start(self, tmp_path: Path) -> None:
        ...
        mock_file.tell.return_value = 10  # Simulate file pointer at position 10
    ...
    def test_image_to_pdf_with_invalid_image(self, tmp_path: Path) -> None:
        ...
        with patch('builtins.open', mock_open(read_data=invalid_image_data)) as mock_file:
            with patch('img2pdf.convert', side_effect=img2pdf.ImageOpenError("Invalid image data")) as mock_convert:
    ...
    def test_image_to_pdf_unexpected_error(self, tmp_path: Path) -> None:
        ...
        with patch('builtins.open', mock_open()) as mock_file_open:
            ...
            with patch('img2pdf.convert', side_effect=RuntimeError("Unexpected error")) as mock_convert:
                with pytest.raises(ValueError) as exc_info:
                    PDFGenerator.image_to_pdf(test_data, output_path)
                
                # Verify the error message is as expected
                assert "Failed to convert image to PDF: Unexpected error" in str(exc_info.value)
    ...
    def test_image_to_pdf_io_error(self, tmp_path: Path) -> None:
        ...
        with patch('builtins.open', return_value=mock_file):
            ...
            with patch('img2pdf.convert', return_value=b'%PDF-1.4\nDummy PDF'):
                ...
                # Verify the error message is as expected
                assert "Failed to convert image to PDF: Disk full" in str(exc_info.value)
    ...
    def test_create_blank_page(self, mock_save: MagicMock, mock_new: MagicMock) -> None:
        """Test creating a blank PDF page."""
        # Setup
        mock_img = MagicMock()
